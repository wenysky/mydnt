    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        public int UpdateForum(int fid, string name, int subforumcount, int displayorder)
        {
            DbParameter[] parms = {
                                        ...
                                        DbHelper.MakeInParam("@name", (DbType)OleDbType.VarChar, 50, name),
        ...
        public DataTable GetForumInformation(int fid)
        {
            ...
            DataTable dt = DbHelper.ExecuteDataset(CommandType.Text, sql, parms).Tables[0];
            dt.Columns[0].ColumnName = "fid";
            return dt;
        }
        ...
        public DataTable GetForumTableBySpecialUser(string username) 
        {
            ...
            if (username != "")
            {
                sql = string.Format("SELECT f1.fid,f1.name,f2.permuserlist,f2.moderators FROM ([{0}forums] f1 LEFT JOIN [{0}forumfields] f2 ON [f1].[fid]=[f2].[fid]) WHERE [f2].[permuserlist] NOT LIKE '' AND [f2].[permuserlist] LIKE '%{1}%'", BaseConfigs.GetTablePrefix, username);
                return DbHelper.ExecuteDataset(CommandType.Text, sql).Tables[0];
            }
            sql = string.Format("SELECT f1.fid,f1.name,f2.permuserlist,f2.moderators FROM ([{0}forums] f1 LEFT JOIN [{0}forumfields] f2 ON [f1].[fid]=[f2].[fid]) WHERE [f2].[permuserlist] NOT LIKE ''", BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteDataset(CommandType.Text, sql).Tables[0];
        }
        ...
        public void SetForumslayer(int layer, string parentidlist, int fid)
        {
            ...
            DbHelper.ExecuteNonQuery(CommandType.Text, "UPDATE [" + BaseConfigs.GetTablePrefix + "forums] SET [parentidlist]=@parentidlist WHERE [fid]=@fid", parms1);
        }
        ...
        {
            string sql = "SELECT [groupid],(SELECT TOP 1 [grouptitle]  FROM [" + BaseConfigs.GetTablePrefix + "usergroups] WHERE [" + BaseConfigs.GetTablePrefix + "usergroups].[groupid]=[" + BaseConfigs.GetTablePrefix + "onlinelist].[groupid]) AS GroupName ,[displayorder],[title],[img] FROM [" + BaseConfigs.GetTablePrefix + "onlinelist] ORDER BY [groupid] ASC";
        ...
        public int UpdateOnlineList(int groupid, int displayorder, string img, string title)
        {
            DbParameter[] parms = { 
                                        DbHelper.MakeInParam("@displayorder", (DbType)OleDbType.Integer, 4, displayorder),
                                        DbHelper.MakeInParam("@title", (DbType)OleDbType.VarChar, 50, title),
                                        DbHelper.MakeInParam("@img", (DbType)OleDbType.VarWChar, 50, img),
...
//            #endregion
        ...
        public void UpdateLastPost(ForumInfo foruminfo, PostInfo postinfo)
        {
            DbParameter[] parms ={
                                     ...
                                     DbHelper.MakeInParam("@lasttitle", (DbType)OleDbType.VarChar, 60, postinfo.Topictitle),
                                     ...
                                     DbHelper.MakeInParam("@lastposter", (DbType)OleDbType.VarChar, 20, postinfo.Poster),
        ...
        public string GetAnnouncePrivateMessageSql()
        {
            return string.Format("SELECT * FROM [{0}pms] WHERE [msgtoid] = 0 ORDER BY [pmid] DESC", BaseConfigs.GetTablePrefix);
        }
        ...
        {
            DbParameter[] parms = { DbHelper.MakeInParam("@rewritename", (DbType)OleDbType.VarWChar, 20, rewriteName) };
