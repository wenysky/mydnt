...
namespace Discuz.Data.Access
{
    public partial class DataProvider : IDataProvider
    {
        ...
        {
            return "SELECT * FROM [" + BaseConfigs.GetTablePrefix + "announcements] ORDER BY [displayorder] DESC,[id] DESC";
        }

        public string GetAnnouncements(int num, int pageid)
        {
            if (pageid == 1)
            {

                return "SELECT TOP " + num + " * FROM [" + BaseConfigs.GetTablePrefix + "announcements] ORDER BY [displayorder], [starttime] DESC, [id] DESC";
            
            }

            return "SELECT TOP " + num + " * FROM [" + BaseConfigs.GetTablePrefix + "announcements]  WHERE [id] NOT IN (SELECT TOP " + ((pageid - 1) * num) + " [id] FROM [" + BaseConfigs.GetTablePrefix + "announcements] ORDER BY [displayorder], [starttime] DESC, [id] DESC) ORDER BY [displayorder], [starttime] DESC, [id] DESC";
        }
        ...
        public int UpdateAnnouncement(int id, string poster, string title, int displayorder, string starttime, string endtime, string message)
        {
            DbParameter[] parms = {
                                        DbHelper.MakeInParam("@displayorder", (DbType)OleDbType.Integer, 4, displayorder),
                                        DbHelper.MakeInParam("@title", (DbType)OleDbType.VarWChar, 250, title),
        ...
        public int DeleteAnnouncement(int id)
        {
            ...
            return DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE FROM [" + BaseConfigs.GetTablePrefix + "announcements] WHERE [id]=@id", parm);
        }
        ...
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iif(ISNULL(MAX(templateid)),0,MAX(templateid)) FROM " + BaseConfigs.GetTablePrefix + "templates"), 0);
        }
        ...
        public int AddErrLoginCount(string ip)
        {
            ...
            return DbHelper.ExecuteNonQuery(CommandType.Text, "UPDATE [" + BaseConfigs.GetTablePrefix + "failedlogins] SET [errcount]=[errcount]+1, [lastupdate]=now() WHERE [ip]=@ip", parms);
        }
        ...
        public int AddErrLoginRecord(string ip)
        {
            ...
            return DbHelper.ExecuteNonQuery(CommandType.Text, "INSERT INTO [" + BaseConfigs.GetTablePrefix + "failedlogins] ([ip], [errcount], [lastupdate]) VALUES(@ip, 1, NOW())", parms);
        }
        ...
        public int ResetErrLoginCount(string ip)
        {
            ...
            return DbHelper.ExecuteNonQuery(CommandType.Text, "UPDATE [" + BaseConfigs.GetTablePrefix + "failedlogins] SET [errcount]=1, [lastupdate]=NOW() WHERE [ip]=@ip", parms);
        }
        ...
        public int DeleteErrLoginRecord(string ip)
        {
            ...
            return DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE FROM [" + BaseConfigs.GetTablePrefix + "failedlogins] WHERE [ip]=@ip OR Datediff(\"n\",[lastupdate], now()) > 15", prams);
        }
        ...
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iif(isnull(MAX(id)), 0,MAX(id)) FROM " + BaseConfigs.GetTablePrefix + "tablelist"), 0);
        }

        public int GetMaxPostTableTid(string posttablename)
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iif(ISNULL(MAX(tid)), 0,MAX(tid)) FROM " + posttablename), 0) + 1;
        }

        public int GetMinPostTableTid(string posttablename)
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iif(ISNULL(MIN(tid)), 0,MAX(tid)) FROM " + posttablename), 0) + 1;
        }
        ...
        public DataTable GetPaymentLogByTid(int pagesize, int currentpage, int tid)
        {
            ...
            if (currentpage == 1)
            {
                sqlstring = "SELECT TOP " + pagesize + " [" + BaseConfigs.GetTablePrefix + "paymentlog].*, [" + BaseConfigs.GetTablePrefix + "users].[username] AS username FROM [" + BaseConfigs.GetTablePrefix + "paymentlog] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "topics] ON [" + BaseConfigs.GetTablePrefix + "paymentlog].[tid] = [" + BaseConfigs.GetTablePrefix + "topics].[tid] LEFT OUTER JOIN [" + BaseConfigs.GetTablePrefix + "users] ON [" + BaseConfigs.GetTablePrefix + "users].[uid] = [" + BaseConfigs.GetTablePrefix + "paymentlog].[uid] WHERE [" + BaseConfigs.GetTablePrefix + "paymentlog].[tid]=" + tid + "  ORDER BY [id] DESC";
            }
            else
            {
        ...
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT iif(ISNULL(MAX(id)), 0,MAX(id)) FROM " + BaseConfigs.GetTablePrefix + "smilies"), 0) + 1;
        }
        ...
        {
            return DbHelper.ExecuteDataset(CommandType.Text, "SELECT TOP 1 * FROM [" + BaseConfigs.GetTablePrefix + "statistics]").Tables[0];
        }
        ...
        {
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT SUM([topics]) AS [topiccount],SUM([posts]) AS [postcount],SUM([todayposts])-(SELECT SUM([todayposts]) from [{0}forums] WHERE [lastpost] < format(now(),'yyyy-mm-dd') AND [layer]=1) AS [todaypostcount] FROM [{0}forums] WHERE [layer]=1", BaseConfigs.GetTablePrefix));
        }
        ...
        public IDataReader GetForumStatistics(int fid)
        {
            ...
            return DbHelper.ExecuteReader(CommandType.Text, string.Format("SELECT SUM([topics]) AS [topiccount],SUM([posts]) AS [postcount],SUM([todayposts])-(SELECT SUM([todayposts]) from [{0}forums] WHERE [lastpost] < format(now(),'yyyy-mm-dd') AND [layer]=1 AND [fid] = @fid) AS [todaypostcount] FROM [{0}forums] WHERE [fid] = @fid AND [layer]=1", BaseConfigs.GetTablePrefix), prams);

            //SELECT SUM([topics]) AS [topiccount],SUM([posts]) AS [postcount],SUM([todayposts])-(SELECT SUM([todayposts]) from [" + BaseConfigFactory.GetTablePrefix + "forums] WHERE [lastpost] < format(now(),'yyyy-mm-dd') AND [layer]=1 AND [fid] = @fid) AS [todaypostcount] FROM [" + BaseConfigFactory.GetTablePrefix + "forums] WHERE [fid] = @fid AND [layer]=1
        }
        ...
        private string GetSearchByPosterSQL(bool spaceenabled, bool albumenabled, int posterid, int posttableid)
        {
            if (posterid > 0)
            {
                ...
                sql.Append(string.Format(@"SELECT DISTINCT [tid], 'forum' AS [datafrom] FROM [{0}posts{1}] WHERE [posterid]={2} AND [tid] NOT IN (SELECT [tid] FROM [{0}topics] WHERE [posterid]={2} AND [displayorder]<0)", BaseConfigs.GetTablePrefix, posttableid, posterid));
                if (spaceenabled)
                {

                    sql.Append(string.Format(@" UNION ALL SELECT [postid],'spacepost' AS [datafrom] FROM [{0}spaceposts] WHERE [uid]={1} ", BaseConfigs.GetTablePrefix, posterid));
                }

                if (albumenabled)
                {

                    sql.Append(string.Format(@" UNION ALL SELECT [albumid],'album' AS [datafrom] FROM [{0}albums] WHERE   [userid]={1}", BaseConfigs.GetTablePrefix, posterid));
                }
                return sql.ToString();
            }
            return string.Empty;
        ...
        public int Search(bool spaceenabled, bool albumenabled, int posttableid, int userid, int usergroupid, string keyword, int posterid, string type, string searchforumid, int keywordtype, int searchtime, int searchtimetype, int resultorder, int resultordertype)
        {
        ...
        {
            string sqlstring = "[actions] LIKE '%" + RegEsc(keyword) + "%' or [reason] LIKE '%" + RegEsc(keyword) + "%' or  [moderatorname] LIKE '%" + RegEsc(keyword) + "%' OR [grouptitle] LIKE '%" + RegEsc(keyword) + "%' OR [fname] LIKE '%" + RegEsc(keyword) + "%'";
            return sqlstring;
        }
        ...
        {
            usernamelist = Utils.ChkSQL(usernamelist);
        ...
        public DataTable GetMailTable(string uids)
        {
            ...
            string strwhere = " WHERE [email] Is Not null AND [uid] IN (" + uids + ")";

            return DbHelper.ExecuteDataset("SELECT [uid],[username],[email] FROM [" + BaseConfigs.GetTablePrefix + "users] " + strwhere).Tables[0];
        }
        ...
        {
            string sql = "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "scheduledevents] WHERE ([key]=@key) AND ([lastexecuted] < DATEADD(\"day\", -7, date()))";
            ...
            string sql1 = "INSERT [" + BaseConfigs.GetTablePrefix + "scheduledevents] ([key], [servername], [lastexecuted]) Values (@key, @servername, @lastexecuted)";
            DbParameter[] parms1 = {
                DbHelper.MakeInParam("@key", (DbType)OleDbType.VarChar, 100, key),
        ...
        public DateTime GetLastExecuteScheduledEventDateTime(string key, string servername)
        {
            ...
            string sql = "SELECT MAX([lastexecuted]) FROM [" + BaseConfigs.GetTablePrefix + "scheduledevents] WHERE [key] = @key AND [servername] = @servername";

            string lastexecuted = DbHelper.ExecuteScalar(CommandType.Text, sql, parms).ToString();


            if (lastexecuted == "" || lastexecuted == null)
            {
                lastexecuted = DateTime.Now.AddDays(-365).ToString();
            }

            return Convert.ToDateTime(lastexecuted);
        }
        ...
        public void UpdateStats(string type, string variable, int count)
        {
            ...
            if (lines == 0)
            {
                ...
                DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms1);
            }
        }
        ...
        public IDataReader GetBestMember(string posttableid)
        {
            ...
            string commandText = string.Format("SELECT [posts],[poster]  From (SELECT [poster], COUNT(1) AS [posts] FROM ["+BaseConfigs.GetTablePrefix+"posts1] WHERE [postdatetime]>=#{2}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster]) ORDER BY [posts] DESC ", BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"));
            return DbHelper.ExecuteReader(CommandType.Text, commandText);
        ...
        public IDataReader GetUsersRank(int count, string posttableid, string type)
        {
            ...
            switch (type)
            {
                ...
                case "digestposts":
                    ...
                    break;
                case "thismonth":
                    commandText = string.Format("SELECT * FROM (SELECT DISTINCT TOP {0} [poster] AS [username], [posterid] AS [uid], COUNT(pid) AS [posts] FROM [{1}posts{2}] WHERE [postdatetime]>=#{3}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster], [posterid]) ORDER BY [posts] DESC", count, BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.AddDays(-30).ToString("yyyy-MM-dd"));
                    break;
                case "today":
                    commandText = string.Format("SELECT * FROM (SELECT DISTINCT TOP {0} [poster] AS [username], [posterid] AS [uid], COUNT(pid) AS [posts] FROM [{1}posts{2}] WHERE [postdatetime]>=#{3}# AND [invisible]=0 AND [posterid]>0 GROUP BY [poster], [posterid]) ORDER BY [posts] DESC", count, BaseConfigs.GetTablePrefix, posttableid, DateTime.Now.ToString("yyyy-MM-dd"));
                    break;
        ...
        public IDataReader GetNoticeByUid(int uid)
        {
            return DbHelper.ExecuteReader(CommandType.Text, "SELECT * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid] = @uid ORDER BY [postdatetime] DESC", DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, uid));
        }
        ...
        public IDataReader GetNoticeByUid(int uid, Noticetype noticetype)
        {
            DbParameter[] parms = { 
                                        DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, uid),
                                        DbHelper.MakeInParam("@type", (DbType)OleDbType.Integer, 4, noticetype),
                                  };
        ...
        public IDataReader GetNoticeByNid(int nid)
        {
            return DbHelper.ExecuteReader(CommandType.Text, "SELECT * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [nid] = @nid ORDER BY [postdatetime] DESC", DbHelper.MakeInParam("@nid", (DbType)OleDbType.Integer, 4, nid));
        }
        ...
        public int GetNoticeNewCountByUid(int uid)
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, string.Format("SELECT COUNT(nid) FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid] = {0} WHERE [new]=1", DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, uid))), 0);
        }

        public int CreateNoticeInfo(NoticeInfo noticeinfo)
        {
            DbParameter[] parms = { 
                                        DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, noticeinfo.Uid),
                                        DbHelper.MakeInParam("@type", (DbType)OleDbType.Integer, 4, noticeinfo.Type),
                                        DbHelper.MakeInParam("@new", (DbType)OleDbType.Integer, 4, noticeinfo.New),
                                        DbHelper.MakeInParam("@posterid", (DbType)OleDbType.Integer, 4, noticeinfo.Posterid),
                                        DbHelper.MakeInParam("@poster", (DbType)OleDbType.VarWChar, 20, noticeinfo.Poster),
                                        DbHelper.MakeInParam("@note", (DbType)OleDbType.VarWChar, 0, noticeinfo.Note),
                                        DbHelper.MakeInParam("@postdatetime", (DbType)OleDbType.DBTimeStamp, 8, noticeinfo.Postdatetime)
                                    };
            DbHelper.ExecuteScalar(CommandType.Text, "INSERT INTO [" + BaseConfigs.GetTablePrefix + "notices] ([uid], [type], [new], [posterid], [poster], [note], [postdatetime]) VALUES (@uid, @type, @new, @posterid, @poster, @note, @postdatetime)", parms);
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT [nid] FROM [" + BaseConfigs.GetTablePrefix + "notices] ORDER BY [nid] DESC"), -1);
        }
        ...
        public bool UpdateNoticeInfo(NoticeInfo noticeinfo)
        {
            DbParameter[] parms = { 
                                        ...
                                        DbHelper.MakeInParam("@note", (DbType)OleDbType.VarWChar, 0, noticeinfo.Note),
                                        DbHelper.MakeInParam("@postdatetime", (DbType)OleDbType.DBTimeStamp, 8, noticeinfo.Postdatetime),
                                        DbHelper.MakeInParam("@nid", (DbType)OleDbType.Integer, 4, noticeinfo.Nid)
                                    };
            return DbHelper.ExecuteNonQuery(CommandType.Text, "UPDATE [" + BaseConfigs.GetTablePrefix + "notices] SET  [uid] = @uid, [type] = @type, [new] = @new, [posterid] = @posterid, [poster] = @poster, [note] = @note, [postdatetime] = @postdatetime  WHERE [nid] = @nid", parms) > 0;
        }

        public bool DeleteNoticeByNid(int nid)
        {
            return DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [nid] = @nid", DbHelper.MakeInParam("@nid", (DbType)OleDbType.Integer, 4, nid)) > 0;
        }

        public bool DeleteNoticeByUid(int uid)
        {
            return DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE *  FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid] = @uid", DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, uid)) > 0;
        }


        public int GetAnnouncementPosterid(int announcementid)
        {
            DbParameter[] parameter = { 
                                        DbHelper.MakeInParam("@id",(DbType)OleDbType.Integer, 4,announcementid)
                                      };
            return Convert.ToInt32(DbHelper.ExecuteScalarToStr(CommandType.Text, "SELECT [posterid] FROM [" + BaseConfigs.GetTablePrefix + "announcements] WHERE id=@id", parameter));
        }

        public IDataReader GetNewNotices(int userid)
        {
            string commandText = "SELECT Top 5 * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid] = @uid AND [new] = 1 ORDER BY [postdatetime] DESC";
        ...
        {
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, string.Format("SELECT COUNT(nid) FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid] = {0} AND [new] = 1", uid)), 0);
        }

        public void UpdateNoticeNewByUid(int uid, int newtype)
        {
            DbParameter[] parms = {                                    
                                    DbHelper.MakeInParam("@new", (DbType)OleDbType.Integer, 4, newtype),
                                    DbHelper.MakeInParam("@uid", (DbType)OleDbType.Integer, 4, uid)
                                };
            DbHelper.ExecuteNonQuery(CommandType.Text, "Update [" + BaseConfigs.GetTablePrefix + "notices] SET [new] = @new WHERE [uid] = @uid", parms);
        }
        ...
        public void DeleteNotice(Noticetype noticetype)
        {
            ...
            {
                sql = "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "notices]";
            }
            DbHelper.ExecuteNonQuery(CommandType.Text, sql);
        }

        public void AddBannedIp(int ip1, int ip2, int ip3, int ip4, string username, string deteline, string expiration)
        {
            DbParameter[] parameters = {
                                         ...
                                         DbHelper.MakeInParam("@admin",(DbType)OleDbType.VarWChar,50,username),
                                         DbHelper.MakeInParam("@dateline",(DbType)OleDbType.VarWChar,50,deteline),
                                         DbHelper.MakeInParam("@expiration",(DbType)OleDbType.VarWChar,50,expiration)
                                       };

            string sql = "INSERT INTO [" + BaseConfigs.GetTablePrefix + "banned](ip1,ip2,ip3,ip4,admin,dateline,expiration) VALUES(@ip1,@ip2,@ip3,@ip4,@admin,@dateline,@expiration)";
        ...
        {
            string sql = "SELECT COUNT(*) FROM [" + BaseConfigs.GetTablePrefix + "banned]";
        ...
        {
            string sql = string.Format("SELECT * FROM [{0}banned] ORDER BY [id] DESC", BaseConfigs.GetTablePrefix);
        ...
        public IDataReader GetBannedIpList(int num, int pageid)
        {
            ...
            //string sql = string.Format("SELECT TOP " + num + " * FROM [{1}banned]  WHERE [id] NOT IN (SELECT TOP " + (pageid - 1) * num + " [id] FROM [{1}banned] ORDER BY [id] DESC) ORDER BY [id] DESC", num, BaseConfigs.GetTablePrefix, (pageid - 1) * num);
            string sql = "";
            if ((pageid - 1) * num == 0)
            {
                sql = string.Format("SELECT TOP {0} * FROM [{1}banned] ORDER BY [id] DESC", num, BaseConfigs.GetTablePrefix);
            }
            else
            {
                sql = string.Format("SELECT TOP {0} * FROM [{1}banned]  WHERE [id] NOT IN (SELECT TOP {2} [id] FROM [{1}banned] ORDER BY [id] DESC) ORDER BY [id] DESC", num, BaseConfigs.GetTablePrefix, (pageid - 1) * num);
            }
            return DbHelper.ExecuteReader(CommandType.Text, sql); 
        }
        ...
        {
            DbParameter[] parameters = {
                                         DbHelper.MakeInParam("@expiration",(DbType)OleDbType.DBTimeStamp, 8, DateTime.Parse(endtime)),
                                         DbHelper.MakeInParam("@id",(DbType)OleDbType.Integer, 4, id)
                                       };

            string sqlstring = "UPDATE [" + BaseConfigs.GetTablePrefix + "banned] SET [expiration] = @expiration WHERE [id]=@id";

            return DbHelper.ExecuteNonQuery(CommandType.Text, sqlstring, parameters);
        }

        public int DeleteBanIp(string bannedidlist)
        {
            if (!Utils.IsNumericArray(bannedidlist.Split(',')))
            {
                return 0;
            }

            string sqlstring = "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "banned] WHERE [id] IN(" + bannedidlist + ")";

            return DbHelper.ExecuteNonQuery(CommandType.Text, sqlstring);
        }

        public int UpdateDisplayOrder(int displayorder, int aid)
        {
            DbParameter[] parameters = {
                                         DbHelper.MakeInParam("@displayorder",(DbType)OleDbType.Integer, 4, displayorder),
                                         DbHelper.MakeInParam("@aid",(DbType)OleDbType.Integer, 4, aid),
            ...
            string sql = "UPDATE  [" + BaseConfigs.GetTablePrefix + "announcements] set [displayorder]=@displayorder WHERE [id]=@aid";
        ...
        {
            string sql = "SELECT * FROM [" + BaseConfigs.GetTablePrefix + "navs]" + (getAllNavigation ? "" : " WHERE [available]=1 ") + " ORDER BY [parentid],[displayorder],[id]";
            return DbHelper.ExecuteReader(CommandType.Text, sql);
        ...
        {
            DbParameter[] param = {
                                         DbHelper.MakeInParam("@parentid",(DbType)OleDbType.Integer,4,nav.Parentid),
                                         DbHelper.MakeInParam("@name",(DbType)OleDbType.VarWChar, 50, nav.Name),
                                         DbHelper.MakeInParam("@title",(DbType)OleDbType.VarWChar, 255, nav.Title),
                                         DbHelper.MakeInParam("@url",(DbType)OleDbType.Char, 255, nav.Url),
                                         DbHelper.MakeInParam("@target",(DbType)OleDbType.Integer, 1, nav.Target),
                                         DbHelper.MakeInParam("@available",(DbType)OleDbType.Integer,1,nav.Available),
                                         DbHelper.MakeInParam("@displayorder",(DbType)OleDbType.Integer,2,nav.Displayorder),
                                         DbHelper.MakeInParam("@level",(DbType)OleDbType.Integer,1,nav.Level)
                                       };
            string sql = String.Format("INSERT INTO [{0}navs] ([parentid],[name],[title],[url],[target],[type],[available],[displayorder],[level]) VALUES(@parentid,@name,@title,@url,@target,1,@available,@displayorder,@level)", BaseConfigs.GetTablePrefix);
        ...
        public void UpdateNavigation(NavInfo nav)
        {
            DbParameter[] param = {
                                         ...
                                         DbHelper.MakeInParam("@displayorder",(DbType)OleDbType.Integer,2,nav.Displayorder),
                                         DbHelper.MakeInParam("@level",(DbType)OleDbType.Integer,1,nav.Level),
                                         DbHelper.MakeInParam("@id",(DbType)OleDbType.Integer,4,nav.Id)
                                       };
            string sql = String.Format("UPDATE [{0}navs] set [name]=@name,[title]=@title,[url]=@url,[target]=@target,[available]=@available,[displayorder]=@displayorder,[level]=@level WHERE id=@id", BaseConfigs.GetTablePrefix);
            DbHelper.ExecuteNonQuery(CommandType.Text, sql, param);
        }

        public void DeleteNavigation(int id)
        {
            DbHelper.ExecuteNonQuery(CommandType.Text, String.Format("DELETE * FROM [{0}navs] WHERE [id] = {1}", BaseConfigs.GetTablePrefix, id));
        }

        public IDataReader GetNavigationHasSub()
        {
            string sql = "SELECT DISTINCT [parentid] FROM [" + BaseConfigs.GetTablePrefix + "navs] ORDER BY [parentid]";
            return DbHelper.ExecuteReader(CommandType.Text, sql);
        }
        ...
        public int GetNoticeCount(int userid, int state)
        {
            DbParameter[] parms = {
									   ...
									   DbHelper.MakeInParam("@type",(DbType)OleDbType.Integer,4, -1),
									   DbHelper.MakeInParam("@state",(DbType)OleDbType.Integer,4,state)
								   };
            string sqlstr;
            if (state == -1)
            {
                sqlstr = "SELECT COUNT(nid) AS [pmcount] FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid]=@userid AND [type]=@type";
            }
            else
            {
                sqlstr = "SELECT COUNT(nid) AS [pmcount] FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [uid]=@userid AND [new]=@state AND [type]=@type";
            } 
           return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, sqlstr, parms).ToString(), 0);
        }

        public string GetBanIpPoster(int id)
        {
            string sql = string.Format("SELECT DISTINCT [admin] FROM [{0}banned] WHERE [id] ={1}", BaseConfigs.GetTablePrefix, id);
            return DbHelper.ExecuteDataset(CommandType.Text, sql).Tables[0].Rows[0]["admin"].ToString();
        }
        public int ReNewNotice(int type, int uid)
        {
            DbParameter[] parms = {
						   DbHelper.MakeInParam("@date",(DbType)OleDbType.DBTimeStamp, 4, DateTime.Now.ToString("yyyy-MM-dd")),
                           DbHelper.MakeInParam("@type",(DbType)OleDbType.Integer, 4, type),
						   DbHelper.MakeInParam("@uid",(DbType)OleDbType.Integer, 4, uid)
					   };
            string sql = "UPDATE [" + BaseConfigs.GetTablePrefix + "notices] SET [new]=1,[postdatetime]=@date WHERE [type]=@type AND [uid]=@uid";
            int noticecount = DbHelper.ExecuteNonQuery(CommandType.Text, sql, parms);
            if (noticecount > 1)
            {

                DbHelper.ExecuteNonQuery("DELETE * FORM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [type]=@type AND [uid]=@uid");
                return 0;
            }
            else
            {
        ...
        {
            DbParameter[] parms = {
						   DbHelper.MakeInParam("@uid",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Uid),
                           DbHelper.MakeInParam("@postdatetime",(DbType)OleDbType.DBTimeStamp, 8, attachPaymentLogInfo.PostDateTime),
                           DbHelper.MakeInParam("@username",(DbType)OleDbType.VarWChar, 20, attachPaymentLogInfo.UserName),
                           DbHelper.MakeInParam("@amount",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Amount),
                           DbHelper.MakeInParam("@netamount",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.NetAmount),
                           DbHelper.MakeInParam("@aid",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Aid),
						   DbHelper.MakeInParam("@authorid",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Authorid)
					   };
            string sql = "INSERT INTO [" + BaseConfigs.GetTablePrefix + "attachpaymentlog] ([uid],[postdatetime],[username],[amount],[netamount],[aid],[authorid]) Value(@uid,@postdatetime,@username,@amount,@netamount,@aid,@authorid);SELECT SCOPE_IDENTITY()";
        ...
        public int UpdateAttachPaymetLog(AttachPaymentlogInfo attachPaymentLogInfo)
        {
            DbParameter[] parms = {
                               ...
                               DbHelper.MakeInParam("@aid",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Aid),
			                   DbHelper.MakeInParam("@authorid",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Authorid),
			                   DbHelper.MakeInParam("@id",(DbType)OleDbType.Integer, 4, attachPaymentLogInfo.Id)
					   };
        ...
        public IDataReader GetAttachPaymetLogByAid(int aid)
        {
            return DbHelper.ExecuteReader(CommandType.Text, "SELECT * [" + BaseConfigs.GetTablePrefix + "attachpaymentlog] WHERE [aid]=@aid", DbHelper.MakeInParam("@aid", (DbType)OleDbType.Integer, 4, aid));
        }

        public int IsBuyAttach(int userid, int aid)
        {
            DbParameter[] parms = {
                           DbHelper.MakeInParam("@aid",(DbType)OleDbType.Integer, 4, aid),
						   DbHelper.MakeInParam("@uid",(DbType)OleDbType.Integer, 4, userid)
					   };
            return Utils.StrToInt(DbHelper.ExecuteScalar(CommandType.Text, "SELECT COUNT(1) FROM [" + BaseConfigs.GetTablePrefix + "attachpaymentlog] WHERE [aid]=@aid AND [uid] = @uid", parms), 0);
        }

        public void DeleteNotice(Noticetype noticetype, int days)
        {
            if (noticetype == Noticetype.All)
                DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE DATEDIFF(\"d\",[postdatetime], NOW()) > " + days);
            else
                DbHelper.ExecuteNonQuery(CommandType.Text, "DELETE * FROM [" + BaseConfigs.GetTablePrefix + "notices] WHERE [type] = " + (int)noticetype + "AND DATEDIFF(\"d\",[postdatetime], NOW()) > " + days);
        }
    }
}
#endif
