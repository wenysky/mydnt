...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public int GetUnauditNewTopicCount(string fidList, int filter)
        {
            ...
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalarToStr(CommandType.Text, commandText, parms));
        }
        ...
        public IDataReader GetUnauditNewPost(string fidList, int ppp, int pageId, int tableId, int filter)
        {
            ...
            //string commandText = string.Format("SELECT TOP {0} {1}  FROM [{2}posts{3}]", ppp, DbFields.POSTS, BaseConfigs.GetTablePrefix, tableId);

            //if (pageId == 1)
            //{
            //    commandText += "  WHERE [invisible]=@filter AND [layer]>0";
            //    if (fidList != "0")
            //        commandText += " AND [fid] IN (@fidlist)";

            //    commandText += " ORDER BY [pid] DESC";
            //}
            //else
            //{
            //    commandText += string.Format("  WHERE [pid] < (SELECT MIN([pid])  FROM (SELECT TOP {0} [pid] FROM [{1}posts{2}]  where [invisible]=@filter AND [layer]>0", pagetop, BaseConfigs.GetTablePrefix, tableId);
            //    if (fidList != "0")
            //        commandText += " AND [fid] IN (@fidlist)";

            //    commandText += " ORDER BY [pid] DESC) AS T)  ORDER BY [pid] DESC";
            //}
        ...
        public void PassAuditNewTopic(string tableId, string ignore, string validate, string delete, string fidList)
        {
            ...
            {
                DbHelper.ExecuteNonQuery(string.Format("UPDATE  [{0}topics]  SET [displayorder]=0 WHERE [tid] IN ({1})",
                                                         BaseConfigs.GetTablePrefix,
                                                         validate));
        ...
        public void PassAuditNewTopic(string postTableName, string tidList)
        {
            ...
            DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE  [{0}]  SET [invisible]=0 WHERE [layer]=0  AND [tid] IN({1})",
                                                                      postTableName,
                                                                      tidList));
            DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE  [{0}topics]  SET [displayorder]=0 WHERE [tid] IN({1})",
                                                                      BaseConfigs.GetTablePrefix,
                                                                      tidList));
            ...
            //更新相关的版块统计信息
            foreach (DataRow dr in DbHelper.ExecuteDataset(CommandType.Text, string.Format("SELECT * FROM [{0}topics] WHERE [tid] IN({1}) ORDER BY [tid] ASC", BaseConfigs.GetTablePrefix, tidList)).Tables[0].Rows)
            {
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}forums] SET [topics] = [topics] + 1, [curtopics] = [curtopics] + 1, [posts]=[posts] + 1, [todayposts]=CASE WHEN DATEPART(yyyy, [lastpost])=DATEPART(yyyy,GETDATE()) AND DATEPART(mm,[lastpost])=DATEPART(mm,GETDATE()) AND DATEPART(dd, [lastpost])=DATEPART(dd,GETDATE()) THEN [todayposts]*1 + 1	ELSE 1 END,[lasttid]={1} ,	[lasttitle]='{2}',[lastpost]='{3}',[lastposter]='{4}',[lastposterid]={5}  WHERE [fid]={6}",
                                                           BaseConfigs.GetTablePrefix,
                                                           dr["tid"].ToString(),
        ...
        public void AuditPost(int tableId, string validate, string delete, string ignore, string fidList)
        {
            ...
            if (!Utils.StrIsNullOrEmpty(validate))
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}posts{1}] SET [invisible]=0 WHERE [pid] IN ({2})",
                                                                  ...
                                                                  tableId,
                                                                  validate), parms);
            //DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}posts{1}] SET [invisible]=0 WHERE [pid] IN ({2})  AND [fid] IN ({3})",
            //                                                  BaseConfigs.GetTablePrefix,
            //                                                  tableId,
            //                                                  validate,
            //                                                  fidList), parms);
                ...
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("DELETE FROM [{0}posts{1}] WHERE [pid] IN ({2})",
                                                                  ...
                                                                  tableId,
                                                                  delete), parms);
            //DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("DELETE FROM [{0}posts{1}] WHERE ([pid] IN({2}))  AND ([fid] IN ({3}))",
            //                                                  BaseConfigs.GetTablePrefix,
            //                                                  tableId,
            //                                                  delete,
            //                                                  fidList), parms);
                ...
                DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}posts{1}] SET [invisible]=-3 WHERE  [pid] IN ({2})",
                                                                  ...
                                                                  tableId,
                                                                  ignore), parms);
            //DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}posts{1}] SET [invisible]=-3 WHERE  [pid] IN ({2})  AND [fid] IN ({3})",
            //                                                  BaseConfigs.GetTablePrefix,
            //                                                  tableId,
            //                                                  ignore,
            //                                                  fidList), parms);
        ...
        public IDataReader GetPostListByCondition(PostpramsInfo postPramsInfo, string postTableId)
        {
            ...
            if (mcci != null && mcci.ApplyMemCached && postPramsInfo.Pageindex <= 5)
        ...
        public IDataReader GetPostList(PostpramsInfo postParmsInfo, string postTableId)
        {
            ...
            //只有在应用memcached的情况下才可以使用主题缓存，则此时为了确保数据同步，必须从主数据库中提取数据进行缓存
            if (mcci != null && mcci.ApplyMemCached && postParmsInfo.Pageindex <= 5)
                return DbHelper.ExecuteReaderInMasterDB(CommandType.StoredProcedure,
