...
namespace Discuz.EntLib
{
    ...
    public class KetamaNodeLocator
    {
        ...
        public KetamaNodeLocator(System.Collections.ArrayList nodes)
            : this(nodes, 160)
        { }

        public KetamaNodeLocator(System.Collections.ArrayList nodes, int nodeCopies)
        {
            ...
            foreach (object node in nodes)
            {
                ...
                for (int i = 0; i < numReps / 4; i++)
                {
                    ...
                    byte[] digest = HashAlgorithm.computeMd5(node.ToString() + i);
                    /** Md5是一个16字节长度的数组，将16字节的数组每四个字节一组，分别对应一个虚拟结点，这就是为什么上面把虚拟结点四个划分一组的原因*/
                    for (int h = 0; h < 4; h++)
                    {
                        long m = HashAlgorithm.hash(digest, h);
                        ketamaNodes[m] = node.ToString();
                    }
                }
            }
        }
