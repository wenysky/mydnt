...
namespace Discuz.EntLib
{
	...
	/// <summary>
	/// 原型取自<a href="http:/www.danga.com/memcached/">http://www.danga.com/memcached/</a>.	///
	/// 支持设置，添加，替换，删除，压缩/解压缩，序列化设置
	/// 将SockIO 对象植入链接池中. 如果该服务器宕机时，后续数据会自动存入有效的服务器中
	/// </summary>
	/// <example>
	...
	public class MemcachedClient 
	{
        ...
        private const string ITEM = "ITEM"; // start of item line from server        
			...
			for(int i = 0; i < servers.Count; i++) 
			{
				...
				try 
				{
					...
					while(true) 
					{
                        ...
                        if (line.StartsWith(STATS) || line.StartsWith(ITEM)) 
						{
							string[] info = line.Split(' ');
							string key    = info[1];
							string val  = info[2];
        ...
        private static ResourceManager _resourceManager = new ResourceManager("Discuz.EntLib.Memcached.StringMessages", typeof(MemcachedClient).Assembly);
