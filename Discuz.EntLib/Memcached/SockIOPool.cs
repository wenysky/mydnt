...
namespace Discuz.EntLib
{
        ...
        public SockIO GetSock(string key, object hashCode)
        {
            ...
            else
            {
                ...
                switch(_hashingAlgorithm)
                {
                    ...
                    case HashingAlgorithm.KetamaHash:
                        //SockIO sock = GetConnection(HashingAlgorithmHelper.KetamaHashAlgurithm(this.Servers, key));
                        //if (sock != null)
                        //    return sock;
                        //hv = HashingAlgorithmHelper.NewHashingAlgorithm(key);
                        break;
            ...
            while(tries++ <= _buckets.Count)
            {
                SockIO sock;
                int bucket = 0;
                if (_hashingAlgorithm == HashingAlgorithm.KetamaHash)
                    sock = GetConnection(HashingAlgorithmHelper.KetamaHashAlgurithm(this.Servers, key));
                else
                {
                    // get bucket using hashcode 
                    // get one from factory
                    bucket = hv % _buckets.Count;
                    if (bucket < 0)
                        bucket += _buckets.Count;
                    sock = GetConnection((string)_buckets[bucket]);
                }
    ...
    public class HashingAlgorithmHelper
    {
        ...
        public static string KetamaHashAlgurithm(ArrayList arrayList, string key)
        {
            KetamaNodeLocator locator = new KetamaNodeLocator(arrayList);
