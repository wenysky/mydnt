...
namespace Discuz.Forum
{
    ...
    {   
        ...
        public static string GetNavigationString(int userid, int useradminid)
        {
            ...
            bool matchCurrentPage = CheckCurrentPageMatch(dt);
            if (dt != null)
            {
                foreach (System.Data.DataRow dr in dt.Rows)
                {
                    nav = ChangeStyleForCurrentUrl(dr, matchCurrentPage);
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        private static bool CheckCurrentPageMatch(DataTable dt)
        {
            foreach (System.Data.DataRow dr in dt.Rows)
            {
                ...
                if (url == "index.aspx")
                {
                    if (url == "index.aspx" && DNTRequest.GetPageName() == "forumindex.aspx" && !System.Web.HttpContext.Current.Request.RawUrl.ToString().Contains("forumindex.aspx") || DNTRequest.GetPageName() == "website.aspx")
                    {
                        return true;
                    }
                }
                else if (!Utils.StrIsNullOrEmpty(url) && System.Web.HttpContext.Current.Request.RawUrl.ToString().Contains(url))
                {
                    return true;
                }
            }
        ...
        /// <returns></returns>
        private static string ChangeStyleForCurrentUrl(DataRow dr, bool matchCurrentPage)
        {
            string nav = dr["nav"].ToString(), url = dr["url"].ToString().Trim(), name = dr["name"].ToString();
            if (url == "index.aspx")
            {
                if (url == "index.aspx" && DNTRequest.GetPageName() == "forumindex.aspx" && !System.Web.HttpContext.Current.Request.RawUrl.ToString().Contains("forumindex.aspx")|| DNTRequest.GetPageName() == "website.aspx")
                {
                    matchCurrentPage = true;
                    return ReplaceCurrentCssClass(dr, ref nav);
                }
            }
            else if (!Utils.StrIsNullOrEmpty(url) && System.Web.HttpContext.Current.Request.RawUrl.ToString().Contains(url))
            {
            ...
            if (Utils.InArray(name,"论坛,社区,BBS") && !matchCurrentPage)
            {
                return ReplaceCurrentCssClass(dr, ref nav);
            }
            return nav;
        ...
        /// <returns></returns>
        private static string ReplaceCurrentCssClass(DataRow dr, ref string nav)
        {
            nav = nav.Replace("class=\"drop", "class=\"current drop");
            if (nav.Length == dr["nav"].ToString().Length)
