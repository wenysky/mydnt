        ...
        private static OnlineUserInfo CreateUser(int uid, int timeout)
        {
            ...
            if (uid > 0)
            {
                ...
                if (ui != null)
                {
                    ...
                    onlineuserinfo.Lastactivity = 0;
                    onlineuserinfo.Verifycode = ForumUtils.CreateAuthStr(5, false);
                    onlineuserinfo.Newpms = short.Parse(PrivateMessages.GetPrivateMessageCount(uid, 0, 1).ToString());
                    onlineuserinfo.Newnotices = short.Parse(Notices.GetNewNoticeCountByUid(uid).ToString());
