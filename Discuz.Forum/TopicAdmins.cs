...
namespace Discuz.Forum
{
    ...
    public class TopicAdmins
    {
        ...
        private static string GetUserListWithTopiclist(string topiclist, int op)
        {
            ...
            List<ShortUserInfo> shortUserInfoList = Discuz.Data.TopicAdmins.GetUserListWithTopicList(topiclist, op, configinfo.Losslessdel);
        ...
        public static int SetDigest(string topiclist, int intValue)
        {
            ...
            if (mount > 0)
            {
                ...
                if (intValue > 0 && !useridlist.Equals("") && Utils.IsNumericList(useridlist))
                    UserCredits.UpdateUserCreditsAndExtCredits(useridlist, CreditsOperationType.Digest, mount);
            }
            return mount;
        }
