...
namespace Discuz.Web.Admin
{
    public class usergrouppowersetting : System.Web.UI.UserControl
    {
        ...
        public void Bind(UserGroupInfo usergroupinfo)
        {
            ...
            if (usergroupinfo.Allowhtml == 1) usergroupright.Items[19].Selected = true; //是否允许html
            if (usergroupinfo.ModNewTopics == 1) usergroupright.Items[20].Selected = true;//发主题是否需要审核
            if (usergroupinfo.ModNewPosts == 1) usergroupright.Items[21].Selected = true;//发回复是否需要审核
        ...
        public void GetSetting(ref UserGroupInfo usergroupinfo)
        {
            ...
            usergroupinfo.Allowhtml = usergroupright.Items[19].Selected ? 1 : 0;    //是否允许html
            usergroupinfo.ModNewTopics = usergroupright.Items[20].Selected ? 1 : 0; //发主题需审核
            usergroupinfo.ModNewPosts = usergroupright.Items[21].Selected ? 1 : 0; //发回复需审核
            if (MallPluginProvider.GetInstance() != null)
            {
