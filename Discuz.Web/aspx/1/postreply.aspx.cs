            ...
            if (ispost)
            {
                ...
                if (sb.Length > 0)
                {
                    ...
                    if (infloat == 1)
                    {
                        ...
                        AddMsgLine("<table cellspacing=\"0\" cellpadding=\"4\" border=\"0\"><tr><td colspan=2 align=\"left\"><span class=\"bold\"><nobr>发表主题成功,但以下附件上传失败:</nobr></span><br /></td></tr></table>");
                }
                else
                {
                    SetMetaRefresh();
                ...
                //SendNotifyEmail(Users.GetShortUserInfo(topic.Posterid).Email.Trim(), postinfo, Utils.GetRootUrl(BaseConfigs.GetForumPath) + "showtopic.aspx?page=end&topicid=" + topicid + "#" + postid);
            }
        }
