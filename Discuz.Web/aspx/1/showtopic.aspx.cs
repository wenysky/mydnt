        ...
        public new void BindPageCountAndId()
        {
            ...
            if (Utils.StrIsNullOrEmpty(onlyauthor) || onlyauthor == "0")
            {
                if (config.Aspxrewrite == 1)
                    ...
                    pagenumbers = Utils.GetPageNumbers(pageid, pagecount, "showtopic.aspx?topicid=" + topicid + "&forumpage=" + forumpageid, 8);

            }
            else
                pagenumbers = Utils.GetPageNumbers(pageid, pagecount, "showtopic.aspx?onlyauthor=" + onlyauthor + "&topicid=" + topicid + "&forumpage=" + forumpageid + "&posterid=" + posterid, 8);

            if (pageid != pagecount)
                nextpage = "<a href=\"" + Urls.ShowTopicAspxRewrite(topicid, pageid + 1) + "\" class=\"next\">下一页</a>";
