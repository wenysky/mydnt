 ...
 function insertText(text, movestart, moveend, select, sel) {
	if(wysiwyg) {
		if(BROWSER.firefox || BROWSER.opera) {
			...
			checkFocus();
			if(!isUndefined(editdoc.selection)) {
				movestart = false;
...
function getunusedattachlist_callback(doc){
	...
	if(data.length>0)
	{
		...
		msg+='<label>';
		msg+='<input type="checkbox" class="pc" checked="" value="'+data[i].aid+'" name="unused[]" id="unused'+data[i].aid+'">';
		msg+='<span title="'+data[i].attachment+'">'+data[i].attachment+'</span></label></p>';
		}
		msg+='</div><div class="o pns cl"><label class="z">';
		msg+='<input type="checkbox" onclick="checkall(this.form, \'unused\', \'chkall\')" checked="checked" class="pc" name="chkall"> 全选</label>';
		msg+='<button onclick="unusedoption(0)" class="pn" value="true" type="button">';
		msg+='<span>忽略</span></button>';
