...
function onSilverlightError(sender, args) {
   ...
   //alert(errMsg); //throw new Error(errMsg);
}
...
function onSilverlightError(sender, args) {
    ...
    alert(errMsg);
    BOX_removesl('silverlightControlHost');
}


function BOX_showsl(e, divheight) {       //显示
    if (document.getElementById(e) == null) { return; }
    BOX_layoutsl(e, divheight);
    window.onresize = function () { BOX_layoutsl(e); } //改变窗体重新调整位置 
}

function BOX_removesl(e) {       //移除
    window.onscroll = null;
    window.onresize = null;
    document.getElementById('BOX_overlay_sl').style.display = "none";
    try
    {
...
function BOX_layoutsl(e, divheight) {       //调整位置
    var a = document.getElementById(e);
    if (document.getElementById('BOX_overlay_sl') == null) { //判断是否新建遮掩层
        var overlay = document.createElement("div");
        overlay.setAttribute('id', 'BOX_overlay_sl');
    ...
    var bo = document.getElementById('BOX_overlay_sl');
    ...
    a.style.top = scrollTop + ((document.documentElement.clientHeight - divheight) / 2) + 'px';
    //a.style.filter = 'progid:DXImageTransform.Microsoft.Alpha(opacity=80,finishOpacity=100,style=0)';
    //a.style.opacity = 0.9;
    a.style.zIndex = 998;
}
...
function isUndefined(variable) {
    return typeof variable == 'undefined' || variable == '[]' ? true : false;
}



//function CreateSilverlight(pluginID, max, rooturl, forumid, authToken) {
