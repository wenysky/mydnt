...
[maxspacephotosize],[allowdebate],[allowbonus],[minbonusprice],[maxbonusprice],[modnewtopics],[modnewposts]) 
VALUES(
...
@Maxsizeperday,@Attachextensions,@Raterange,@Maxspaceattachsize,@Maxspacephotosize,@Allowdebate,@Allowbonus,@Minbonusprice,@Maxbonusprice,@ModNewTopics,@ModNewPosts)
GO
...
[maxspacephotosize]=@Maxspacephotosize,[modnewtopics]=@ModNewTopics,[modnewposts]=@ModNewPosts
...
IF @orderby = 'downloads'
	BEGIN
		EXEC('SELECT  TOP  '+ @count+'  attach.tid,attach.[filename],attach.[attachment],topic.title FROM [dnt_attachments] AS attach left join [dnt_topics] AS topic ON attach.tid = topic.tid WHERE attach.width > 360  AND height > 240  AND attach.tid in (SELECT  TOP  '+ @count+'  tid FROM [dnt_topics]  WHERE  fid in  ('+@fidlist+')  AND [displayorder] >=0  AND [attachment]=2  ORDER  BY  [VIEWS] DESC)  ORDER  BY [downloads] DESC')
	END
ELSE
	BEGIN
		EXEC('SELECT  TOP  '+ @count+'  attach.tid,attach.[filename],attach.[attachment],topic.title FROM [dnt_attachments] AS attach left join [dnt_topics] AS topic ON attach.tid = topic.tid WHERE attach.width > 360  AND height > 240  AND attach.tid in (SELECT  TOP  '+ @count+'  tid FROM [dnt_topics]  WHERE  fid in ('+@fidlist+')  AND [displayorder] >=0  AND [attachment]=2  ORDER  BY  [tid] DESC)  ORDER  BY [aid] DESC')
	END